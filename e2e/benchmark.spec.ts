    ...
    test(`route: ${routeId}`, async ({ page }) => {
      ...
      page.on('console', (msg) => {
        ...
        if (text.startsWith('[benchmark:summary]')) {
          ...
          try {
            summaries.push(JSON.parse(json) as BenchmarkSummary);
          } catch {
      ...
      // Wait for route to complete (route duration + 10s buffer)
      ...
      const base = (baseline as Record<string, { avgFps: number; p1Fps: number }>)[routeId];
      if (base) {
