...
export const QuestTriggerSchema = z.discriminatedUnion('type', [
  z.object({
    type: z.literal('roadside'),
    distanceRange: z.tuple([z.number(), z.number()]),
  }),
...
export const QuestDefinitionSchema = z
  .object({
    ...
    branches: z
      .object({ A: QuestBranchSchema, B: QuestBranchSchema })
      .optional(),
    prerequisites: z.array(z.string()).optional(),
    reward: QuestRewardSchema,
  })
  .refine((q) => q.steps || q.branches, {
    message: 'Quest must have either steps or branches',
  })
  .refine((q) => q.tier === 'micro' || q.branches, {
